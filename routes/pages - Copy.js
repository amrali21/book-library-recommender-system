  ...
  router.get('/login', ifLoggedin, (req,res,next) => {
   ...
   //   console.log("current username: " + name); // CMT
    ...
    /* Start of website pages */
        ...
        // [EDITED] routes were all defined here for no reason. I cut and pasted them out.
...
//   };
  ...
  // LOGIN PAGE. take token stored in browser (if exists) and ..?
  router.post('/login', (req, res, next) =>  { console.log('hellllllooooooo'); next(); } ,ifLoggedin, [
    body('user_email').custom((value) => {
        console.log('fetching mail hi');
        return db.execute('SELECT `email` FROM `users_table` WHERE `email`=?', [value])
        .then(([rows]) => {
            if(rows.length == 1){
                return true;
    ...
    console.log('fetching validation result hi');
    const validation_result = validationResult(req);
