...
router.use(cookieSession({
  ...
  router.get('/', ifNotLoggedin, (req,res,next) => {
    ...
    /* Start of website pages */
        ...
        {
            res.render("index", {PageTitle: 'Booker',  name: req.session.name /*CMT*/, is_admin: req.session.is_admin, isAuthenticated: req.session.isLoggedIn});
        ...
        router.get("/book/:productId", (req, res, next) =>
        {
            ...
            Books_recommended.then(() =>
            {
                recommendation.rexx.then((recommended_books_ids) =>
                {
                    ...
                    {
                        res.render("product-single", {PageTitle: data[0][0].Book_Title, books: data[0], no_of_books: data[1], err: data[2], name: req.session.name/*rows[0].name*/, is_admin: req.session.is_admin, isAuthenticated: req.session.isLoggedIn});
        ...
        router.get("/Genres/:genre", (req, res, next) =>
        {
            ...
            {
                res.render("genres", {PageTitle: genre, books: data[0], genres: genres, no_of_books: data[1], err: data[2],  name: req.session.name /*CMT*/, is_admin: req.session.is_admin, isAuthenticated: req.session.isLoggedIn});
            });
        });
        ...
        router.get("/Top-Rated", (req, res, next) =>
        {
            ...
            {
                res.render("Top-Rated", {PageTitle: 'Top Rated', books: data[0], genres: genres, no_of_books: data[1], err: data[2],  name: req.session.name /*CMT*/, is_admin: req.session.is_admin, isAuthenticated: req.session.isLoggedIn});
            });
        });
        ...
        router.get("/search/:searched_book_name", (req, res, next) =>
        {
            ...
            {
                res.render("genres", {PageTitle: PageTitle, genres: genres, books: data[0], no_of_books: data[1], err: data[2],  name: req.session.name /*CMT*/, is_admin: req.session.is_admin, isAuthenticated: req.session.isLoggedIn});
            });
        });
        ...
        {
            res.render("contact", {PageTitle: 'Contact',  name: req.session.name /*CMT*/, is_admin: req.session.is_admin, isAuthenticated: req.session.isLoggedIn});
        });

        //view about page
        router.get("/about", (req, res, next) =>
        {
            res.render("about", {PageTitle: 'About',  name: req.session.name /*CMT*/, is_admin: req.session.is_admin, isAuthenticated: req.session.isLoggedIn});
        });
        ...
        if (rows[0].is_admin == 1)
        {
            ...
            {
                res.render("add-new-book", {PageTitle: 'Add New Book To Database', genres: genres, name: req.session.name /*CMT*/, is_admin: req.session.is_admin,isAuthenticated: req.session.isLoggedIn});
            });
        };
